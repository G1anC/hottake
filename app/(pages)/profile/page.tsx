...
export default async function Profile() {
	...
	const Reviews = async ({ reviews }: { reviews: Review[] }) => {
		return (
			<div className="w-full h-full flex flex-col min-h-0">
				...
				<div className="flex-1 overflow-y-auto pr-4 space-y-12 min-h-0">
					{reviews.length > 0 ? (
							...
							return (
								<div key={index} className="">
									<div className="flex justify-between">
										...
										<NoteDisplay note={review.note} key={index} />
									</div>
								</div>
							)
						}
	...
	return (
		<div className="h-screen text-white w-screen overflow-hidden">
				...
				<div className="flex-1 bg-[#0c0c0e] flex flex-col w-full px-48 pb-8 items-center overflow-hidden z-10 min-h-0">
					{/* <div className="w-full min-h-1/5 z-50 mt-8 flex justify-center shrink-0 gap-2">
						{bigFiveAlbums.map((album, index) => {
							...
							return (
								<img
									className="aspect-square z-50 w-full max-w-180 rounded-lg"
					...
					<div className="w-full flex justify-between space-x-8 pt-8 shrink-0">
